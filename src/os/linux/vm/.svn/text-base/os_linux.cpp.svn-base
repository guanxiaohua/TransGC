...
if(TransGC&&UseParkUnpark){
 //by tony 2010-09-29 instrumenting park()
 if(jt->isMonitored())
 	{
//	gclog_or_tty->print_cr("[TransGC-GC]Parker::park() called! thread id:%d, monitored:%d", thread->osthread()->thread_id(),jt->isMonitored());
	
	if(jt->hasValidTransID())//by tony 2010-09-21 Only the transaction threads are monitored.
		{												
//			 	tty->print_cr("[TRANS-END-PARK] a transaction(%d) is finished by thread %d",jt->transID(), thread->osthread()->thread_id());
				if(DebugTypeGenGC){
					gclog_or_tty->print_cr("[TRANS-END-PARK] a transaction(%d) is finished by thread %d",jt->transID(), thread->osthread()->thread_id());
					tty->print_cr("[TRANS-END-PARK] a transaction(%d) is finished by thread %d",jt->transID(), thread->osthread()->thread_id());
					}
				((GenCollectedHeap*)Universe::heap())->set_transEden_collectable(jt->transEdenID(), jt->transID(),jt->thread_counts_in_transaction());				
	 ...
	 //tony-> 2011/02/09 make sure that we captuer all the allocations.
				  if(UseTLAB)
					  thread->tlab().clear_before_allocation();//retire the tlab allocated, even the thread is not regarded as transaction or used to be, but then forced to the local
					  											//we should retire the tlab now because the thread might be used again for a new transaction.
    //<-tony 2011-02-09
