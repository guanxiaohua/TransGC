...
//   __ call_VM(noreg, CAST_FROM_FN_PTR(address, InterpreterRuntime::post_method_entry_tony));
...
void TemplateTable::_new() {
 ...
 //tony-> 2011/02/09 make sure that we captuer all the allocations.
  /* --[ Neo 05/04/2007 ]-------------------------------
   *  Force everything goes to slow path in the 
   *  interpreter
   * ------------------------------------------------ */
...
//@@PENG: marker bytecodes templates, same with i386 and x64
void TemplateTable::markerenter() {
//assert(false,"where are we in the stack?");
  transition(vtos, vtos);
  __ get_unsigned_2_byte_index_at_bcp(rbx, 1);
  __ call_VM(noreg, CAST_FROM_FN_PTR(address, 
             SharedRuntime::markerenter), rbx); // @@PENG	
}

void TemplateTable::markerexit() {
  transition(vtos, vtos);
	
  __ get_unsigned_2_byte_index_at_bcp(rbx, 1);
  __ call_VM(noreg, CAST_FROM_FN_PTR(address, 
             SharedRuntime::markerexit), rbx); // @@PENG
}
