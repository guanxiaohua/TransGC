...
/*tony 09/12/2009 Detecting remote objects, Tony*/ 
  ...
  juint    _live_count;         // how many live count at last
  juint    _live_size;	  	//
  	
  double   _survive_rate;       //Feng
 
  bool     _is_objArray;        // True iff is an obj array.
  bool     _is_remote;          //Added by Feng. Jan, 2006
 // bool	   _is_runnable;  //added by Tony, 04.2010
 // bool     _is_app;             // True iff is an application class
 // bool	   _is_shortlived;
  juint 	_residual_counts; //by tony 2011-01-13 how many objects of this kclass has  survived a transGC?
  juint     _alloc_count_transgc;//this will be added up upone allocation
  bool     _is_residual; //by tony 2010-11-04 analysing the residuals
  bool 	   _is_traced;   //by tony 2010-11-22 adding the _is_traced flag in Klass, indicating to the run time tracer whether or not to trace method call of this class.
/*-tony 09/12/2009*/
 ...
 /*tony 09/12/2009 Detecting remote objects, Tony*/ 
  void set_remote_flag(bool remote)  {_is_remote = remote;  }
  bool is_remote()  { return _is_remote; }

	//by tony 2011/01/13 check the residuals.
  juint residual_counts() {return _residual_counts;}
  void  set_residual_counts(juint c){_residual_counts=c;}

  juint allocation_counts(){return _alloc_count_transgc;}

  void  set_allocation_counts(juint c){_alloc_count_transgc=c;}
  //<---by tony 2011/01/13 check the residuals.
  

  void set_residual_flag(bool residual)  {_is_residual = residual;  }
  bool is_residual()  { return _is_residual; }
   ...
   void set_traced(bool traced){_is_traced=traced;}
    //<-tony 2010-11-21
  //void set_runnable_flag(bool runnable){_is_runnable=runnable;} 
//  bool is_runnable(){return _is_runnable;}
//  void set_shortlived_flag(bool flag)  { _is_shortlived = flag; }
 // bool is_shortlived()  { return _is_shortlived; }

 // void set_is_app(bool b) { _is_app = b; }
 // bool is_app()                     const { return _is_app; }
