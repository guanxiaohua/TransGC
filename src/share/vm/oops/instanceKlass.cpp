...
template <class T> void assert_is_in_closed_subset(T *p) {
  ...
  if (!oopDesc::is_null(heap_oop)) {
    ...
    if(!Universe::heap()->is_in_closed_subset(o)&&DebugTypeGenGC)
		{
		gclog_or_tty->print_cr("[TONY-assert_is_in]%p pointed by %p should be in heap, try transEden(%d)",o,p,Universe::heap()->get_trans_eden_id(o));
    	}
