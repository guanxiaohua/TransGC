...
#endif
 //tony-> 2010-11-15 making the method call trace dynamic by adding a signal handler to the jvm.
 //SIGUSR1=10
#define SIGTONY SIGUSR1
#define SIGTONY2 SIGUSR2 
  ////by tony 2011/01/21, SIGUSR2=12, this is for a manually forced full GC at some time.
 //<-tony 2010-11-15
  ...
  while (true) {
    ...
    switch (sig) {
	  ...
	  case SIGTONY:
	  		{
  				...
  				PrintAllMethodNames=!PrintAllMethodNames;
				tty->print_cr("SIGTONY received(%d), setting PrintAllMethodNames to be %d",sig,PrintAllMethodNames); 
				break;
	  ...
	  case SIGTONY2:
		    ...
		    if(TransGC)
		    	{
				tty->print_cr("SIGTONY2 received(%d),force a fullGC",sig); 
				GenCollectedHeap* gch=(GenCollectedHeap*)(Universe::heap());
				unsigned int gc_count_before      = gch->total_collections();
				unsigned int full_gc_count_before = gch->total_full_collections();
				int maxlevel=gch->n_gens()-1;
				VM_GenCollectFull op(gc_count_before, full_gc_count_before,
                         GCCause::_full_gc_alot,maxlevel);
			    VMThread::execute(&op);
				gclog_or_tty->print_cr("A full GC should occure here.");
				break;
		    	}
...
void os::signal_init() {
  if (!ReduceSignalUsage) {
	...
	os::signal(SIGTONY, os::user_handler());
	os::signal(SIGTONY2,os::user_handler());//by tony 2011/01/21
  }
}
