...
if(!obj->is_oop()&&DebugTypeGenGC){
	gclog_or_tty->print_cr("error in Handles.cpp: obj(%p) is not an oop! which is supposed in gen(%i).",(HeapWord*)obj,Universe::heap()->addr_to_gen_id(obj));
}
