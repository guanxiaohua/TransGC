...
const char*  Arguments::_gc_log_filename        = NULL;
 //tony-> 2010-09-02 Parameterize the transaction boundary strings
const char*  Arguments::_transaction_start_method_string ="sun/misc/Unsafe.unpark";
const char*  Arguments::_transaction_end_method_string ="sun/misc/Unsafe.park";
const char*  Arguments::_transaction_parent_thread_name="RMI TCP Accept-1098";//"RMI TCP Accept-0";//RMI TCP Accept-1098"
int Arguments::_transaction_start_method_position_in_string=0;
int Arguments::_transaction_end_method_position_in_string=15; //the position of '.' in the string
...
//const char*  Arguments::_transaction_start_method_string ="sun/misc/Unsafe.unpark";
//const char*  Arguments::_transaction_end_method_string ="sun/misc/Unsafe.park";
//const char*  Arguments::_transaction_parent_thread_name="RMI TCP Accept-0";

	else if (match_option(option, "-XXTransStart:", &tail)) {
      _transaction_start_method_string= strdup(tail);
	  //now split the mark
	  int pos=strlen(_transaction_start_method_string)-1;
	  while(pos>0&&_transaction_start_method_string[pos]!='.')
	  		pos--;
	  if(pos>0)
	  		_transaction_start_method_position_in_string=pos;
	  else
		  	_transaction_start_method_position_in_string=0;
	  jio_fprintf(defaultStream::error_stream(),
                    "Transaction start method name: %s\n",_transaction_start_method_string+_transaction_start_method_position_in_string+1 );
   
    }
	 else if (match_option(option, "-XXTransEnd:", &tail)) {
      _transaction_end_method_string= strdup(tail);

	  	  //now split the mark
	  int pos=strlen(_transaction_end_method_string)-1;
	  while(pos>0&&_transaction_end_method_string[pos]!='.')
	  		pos--;
	  if(pos>0)
	  		_transaction_end_method_position_in_string=pos;
	  else
		  	_transaction_end_method_position_in_string=0;

  	  jio_fprintf(defaultStream::error_stream(),
                    "Transaction end method name: %s\n",_transaction_start_method_string+_transaction_end_method_position_in_string+1 );

    }
	 else if (match_option(option, "-XXTransParentThread:", &tail)) {
      _transaction_parent_thread_name= strdup(tail);
	  jio_fprintf(defaultStream::output_stream(),"_transaction_parent_thread_name:%s",_transaction_parent_thread_name);
...
//if(TransGC) //this maybe not available yet.

      FLAG_SET_CMDLINE(bool, ExtendedDTraceProbes, true);
      FLAG_SET_CMDLINE(bool, DTraceMethodProbes, true);
      FLAG_SET_CMDLINE(bool, DTraceAllocProbes, false);
      FLAG_SET_CMDLINE(bool, DTraceMonitorProbes, false);
...
/*-tony 09/08/2009*/
 ...
 /*tony 11/16/2009 Porting Type Based GC, Tony*/ 
	 else if (match_option(option, "-XX:+UseTypeGenGC", &tail)){
      FLAG_SET_CMDLINE(bool, UseTypeGenGC, true);
	 }
	 else if (match_option(option, "-XX:-UseTypeGenGC", &tail)){
      FLAG_SET_CMDLINE(bool, UseTypeGenGC, false);
	 }
 /*-tony 11/16/2009*/
