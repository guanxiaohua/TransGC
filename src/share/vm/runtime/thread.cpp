...
void Thread::oops_do(OopClosure* f) {
	//	if(DebugTypeGenGC){
	//	gclog_or_tty->print_cr("Thread::oops_do 0");
	//}
	active_handles()->oops_do(f);

	//	if(DebugTypeGenGC){
	//	gclog_or_tty->print_cr("Thread::oops_do 1");
	//}
	...
	//  	if(DebugTypeGenGC){
	//	gclog_or_tty->print_cr("Thread::oops_do 2");
	//}
	handle_area()->oops_do(f);

	//  	if(DebugTypeGenGC){
	//	gclog_or_tty->print_cr("Thread::oops_do 3");
	//}
}
...
void JavaThread::initialize() {
	...
	_transID=-1;
	_thread_counts_in_transaction=1; //by tony 2010-09-27 investigating the residual of the transactions
	_transEdenID=-1;
	_java_call_counter=0;
	_call_exits=0;
	set_remote_call_flag(false); //FIXME:could be automaticaly remote if the parent thread is remote on
	set_remote_call_counter(0);
	_pooled=false;
	_transStartThread=false;
	_monitor_filter=false; //by tony 2010-09-16
	/*-tony 07/19/2009*/
...
//@@PENG: initialize JavaThread for marker framework
void JavaThread::init_marker_support(bool main_thread) {
	// both CompileThread and LowMemoryDetectorThread inherits JavaThread
	// we only initialize marker data for plain JavaThread that are part
	// of the Java program.
	if (!is_Compiler_thread() && !is_hidden_from_external_view()) {
		_tid = main_thread ? os::Linux::gettid() : osthread()->thread_id();
#ifdef TWO_WAY_MARKER
    _mkstack = new markerStack(_tid); // cache the stack locally		
#endif

    //_client_tag.resize(5 /*clientCount()*/); //@@PENG //by tony 2011/02/28
	}
}
...
//2010-10-7 tony, we insert our check code in a function body
void JavaThread::transCheckBeforeRun(){
	//tony-> 2010-10-7 removed from run() to right before entry()
	/*tony 07/02/2009 try to insert our code in the interpreter*/ 

	Thread* tonythread=(Thread*)this;
	...
	set_remote_call_flag(false); //FIXME:could be automaticaly remote if the parent thread is remote on
	set_remote_call_counter(0);
	_method_call_tracing=true;
	//check the _monitor_filter value of the parent;
	//Only the transaction threads are monitored, so we no longer inherit the monitor attribution any more //by tony 2010-09-21
	// if(_parent->is_Java_thread()&&((JavaThread*)_parent)->isMonitored())
	//	   _monitor_filter=true; //this flag is used to decide whether sharedRuntime will monitor the method call or not.


	int ostid_parent=_parent->osthread()->thread_id();//we added _parent for convenience
	/*-tony 07/19/2009*/
	{
		ResourceMark rm; //rm is a must to call the get_names by tony, get_thread_name_unsafe needs the resource released.
		const char* name=this->get_thread_name_unsafe();
		//	const char* pname=this->get_parent_name_unsafe();
		//  tty->print_cr("TransGC: a new thread %s(tid=%d)! parent:(tid=%d name:%s)",name!=NULL?name:"null",getpid(),ostid,ostid_parent,pname!=NULL?pname:"null");//, this->get_thread_name()); 
		tty->print_cr("TransGC: a new thread: %s(tid=%d),parent:(tid=%d)!",name!=NULL?name:"null",ostid,ostid_parent);//, this->get_thread_name()); 
		gclog_or_tty->print_cr("TransGC: a new thread: %s(tid=%d),parent:(tid=%d)!",name!=NULL?name:"null",ostid,ostid_parent);//, this->get_thread_name()); 

		//now check for jboss tracing, removing the need of monitoring the idling thread, they are just pooling threads active from time to time.
	 	//for jboss
		if(strstr(name,"ScannerThread")
			||strstr(name,"ContainerBackgroundProcessor")
			||strstr(name,"JBossMQ Cache Reference Softner")
			||strstr(name,"Timer")
			){
				_method_call_tracing=false;
				if(DebugTypeGenGC){
				   				tty->print_cr("TransGC:  _method_call_tracing set to false");
						gclog_or_tty->print_cr("TransGC:  _method_call_tracing set to false");
				}
			}

		
		//now check if the tcp accept thread has been created or not
		if(!Universe::isFoundTCPAcceptThread())
		{
			//compare the name.
			//    	if(strcmp(name,"RMI TCP Accept-0")==0){
			if(strcmp(name,Arguments::transaction_parent_thread_name())==0){ //by tony 2010-09-02 
				Universe::setTCPAcceptThreadID(ostid);
			}
		}
			else{
				//tcp accept thread already created, here we assign the transaction ID to the created thread
				if(ostid_parent==Universe::getTCPAcceptThreadID())
				{						
					
					reset_thread_counts_in_transaction();
					setMonitored(true);//we monitor method calls for the transaction thread family.//by tony 2010-09-16
					resetPooled();
					//tony-> 2010-09-21 Enabling TLAB
					//			 ((Thread*)this)->tlab().clear_before_allocation();//here we discard the former TLAB
					//<-tony 2010-09-21
					if(ThreadRunAsTransaction)
						{setTransID(Universe::getNewTransID());
						tty->print_cr("[TRANS-START-THREADRUN]A new transaction(%d) identified by thread %d.",transID(),ostid);
						gclog_or_tty->print_cr("[TRANS-START-THREADRUN]A new transaction(%d) identified by thread %d.",transID(),ostid);
						}
				}
				//tony-> 2010-09-21
				else
				{
					...
					if(CountChildrenForTrans&&_parent->is_Java_thread()&&((JavaThread*)_parent)->hasValidTransID())
					{
						//we arrive here means that this thread is offsprings of the transaction thread

						//if the lower generation of the monitored thread family.
						//TransactionThreadFather
						//   TransactionThread <-monitored 
						//      offsprings of TransactionThread  <--- 
						// then the offsprings inherit the transID at birth. the offsprings of the transaction threads are not monitored for method
						// calling. they only allocate into the transEdenm,and are not responsible for triggering the gc.				
						setTransID(((JavaThread*)_parent)->transID());
//also inherite the transEdenID			
						setTransEdenID(((JavaThread*)_parent)->transEdenID());
						((JavaThread*)_parent)->inc_thread_counts_in_transaction(); //by tony 2010-09-27 investigating the residual of the transactions
						//tony-> 2010-09-21 Enabling TLAB
						// ((Thread*)this)->tlab().clear_before_allocation();//here we discard the former TLAB, enabling it to allocate a TLAB on next allocation request.
						//<-tony 2010-09-21


						tty->print_cr("TransGC: this thread belongs to transaction %d!",transID());
						gclog_or_tty->print_cr("TransGC: this thread belongs to transaction %d!",transID()); 
	...
	void JavaThread::thread_main_inner() {
		...
		if (!this->has_pending_exception() && !java_lang_Thread::is_stillborn(this->threadObj())) {
			...
			HandleMark hm(this);
			if(TransGC)
				transCheckBeforeRun();//by tony 2010-10-7, the transgc related code
	...
	void JavaThread::exit(bool destroy_vm, ExitType exit_type) {
		...
		if(TransGC){
				...
				//tty->print_cr("TransGC: thread(tid=%d) quitting...,remote_call_counter=%d",ostid,remote_call_counter());//, this->get_thread_name()); 
				//gclog_or_tty->print_cr("TransGC: thread(tid=%d) quitting...,remote_call_counter=%d",ostid,remote_call_counter());//, this->get_thread_name()); 

				if(isMonitored()&&hasValidTransID()) //by tony 2010-09-21 only the transaction threads are monitored.
				{
					tty->print_cr("[TRANS-END-THREAD-EXIT] a transaction(%d) is finished by thread %d, thread exiting...",transID(), ostid);
					gclog_or_tty->print_cr("[TRANS-END-THREAD-EXIT] a transaction(%d) is finished by thread %d, thread exiting...",transID(), ostid);
					//tony FIXME: need to do something if this is the transactionStartThread.
					((GenCollectedHeap*)Universe::heap())->set_transEden_collectable(transEdenID(),transID(),thread_counts_in_transaction());
					resetTransID();
					setPooled();
					//We need to invok the remote collection for the transaction area.
				}
	...
	void JavaThread::oops_do(OopClosure* f) {
		...
		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("JavaThread::oops_do 0");
		//}
		// Traverse the GCHandles
		Thread::oops_do(f);

		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("JavaThread::oops_do 1");
		//}
		...
		if (has_last_Java_frame()) {
			...
			//  if(DebugTypeGenGC){
			//	gclog_or_tty->print_cr("JavaThread::oops_do 2");
			//}
			...
			//  if(DebugTypeGenGC){
			//	gclog_or_tty->print_cr("JavaThread::oops_do 3");
			//}
			...
			//  if(DebugTypeGenGC){
			//	gclog_or_tty->print_cr("JavaThread::oops_do 4");
			//}
		...
		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("JavaThread::oops_do 5");
		//}
	...
	//this is appened by tony, an unsafe version of get_thread_name
	const char* JavaThread::get_thread_name_unsafe() const {
	...
	const char* JavaThread::get_parent_name_unsafe() const {
...
#define ALL_JAVA_THREADS(X) for (JavaThread* X = _thread_list; X; X = X->next())
	...
	jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
		...
		/*tony 08/31/2009 Simulating JVMTI Enablings, Tony*/ 
		//start monitoring method entry/exit
		//  VMInited=true;
		/*-tony 08/31/2009*/

		{
			...
			if (InitializeJavaLangSystem) {
				...
				/*tony 09/12/2009 Detecting remote objects, Tony*/ 
				//done in systemDictionary.
				//	   initialize_class(vmSymbolHandles::java_rmi_Remote(),  CHECK_0);
				/*-tony 09/12/2009*/
	...
	//      > release JNI handle blocks, remove
	stack guard pages
	...
	*/
	...
	void Threads::oops_do(OopClosure* f) {
		...
		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("Threads::oops_do 0");
		//}
		...
		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("Threads::oops_do 1");
		//}
		VMThread::vm_thread()->oops_do(f);


		//  if(DebugTypeGenGC){
		//	gclog_or_tty->print_cr("Threads::oops_do 2");
		//}
	}
