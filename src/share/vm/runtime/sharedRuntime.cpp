  ...
  symbolOop sig = method->signature();
   ...
   thread->inc_java_call_counter(); 
	...
	if(TransGC)
		...
		ResourceMark rm;
	if(DebugTypeGenGC&&PrintAllMethodNames&&thread->is_method_call_tracing())
		{
				char* sign=sig->as_C_string();
				char* cn=kname->as_C_string();
				char* mn=name->as_C_string();
				//filtering out the common String and Character classes, this also includes all classes beginiing with such names like java/lang/StringBuffer, etc.
				if(!strstr(cn,"java/lang/String")&&!strstr(cn,"java/lang/Character")){
				  tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());				   
				  gclog_or_tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());		
				}

		}

 if(TransGC&&!UseParkUnpark&&thread->isMonitored())
 	{

	  klassOop k = method->method_holder();
	  instanceKlass* ik = (instanceKlass*) k->klass_part();

 	//here we are looking for org/apache/tomcat/util/net/PoolTcpEndpoint::recycleWorkerThread
 	//or org/apache/tomcat/util/net/MasterSlaveWorkerThread::await()
//	if(jt->hasValidTransID())//by tony 2010-09-21 Only the transaction threads are monitored.
/*
	if(DebugTypeGenGC&&(strcmp(cn,"org/apache/tomcat/util/net/MasterSlaveWorkerThread")==0||strcmp(mn,"performTask")==0))
		{

				  tty->print_cr("[TransGC-GC]klass:%s,traced:%d",ik->external_name(),ik->is_traced());
				  tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());				   
	//			  gclog_or_tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());		
		}
		*/
	if(ik->is_traced())
		{
		//the class is registered to be traced.

  		  //tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());				   
			char* mn=name->as_C_string();
			if(strcmp(mn,"await")==0)
					{	//waken from await, a new transaction.											
							if(DebugTypeGenGC){
//								gclog_or_tty->print_cr("[TRANS-END-AWAIT] a transaction(%d) is finished by thread %d",thread->transID(), thread->osthread()->thread_id());
										  tty->print_cr("[TRANS-END-AWAIT] a transaction(%d) is finished by thread %d",thread->transID(), thread->osthread()->thread_id());
								}
				  //tony-> 2011/02/09 make sure that we captuer all the allocations.
				  if(UseTLAB)
					  thread->tlab().clear_before_allocation();//retire the tlab allocated.
				  //<-tony 2011-02-09
				  

							if(thread->hasValidTransID())
								{
								((GenCollectedHeap*)Universe::heap())->set_transEden_collectable(thread->transEdenID(), thread->transID(),thread->thread_counts_in_transaction());				
								thread->resetTransID();
								}
								thread->setPooled();
					}
   ...
   //judge if this is the re-use of a previous pooled thread
   //by tony 2010-10-4, we will do this check in the allocation part.
#if 0
    if(thread->isMonitored()&&thread->isPooled()&&!thread->hasValidTransID())
		{

				/*
				FIXME: the new transaction operations needed here
				*/
				thread->setTransID(Universe::getNewTransID());
				thread->resetPooled();
				thread->reset_thread_counts_in_transaction();
				tty->print_cr("[TRANS-START-UNPARK]A new transaction(%d) identified by the pooled thread %d.",thread->transID(),thread->osthread()->thread_id());
				gclog_or_tty->print_cr("[TRANS-START-UNPARK]A new transaction(%d) identified by the pooled thread %d.",thread->transID(),thread->osthread()->thread_id());
		}
#endif 
   	
	//check if it's a remote call  
/*
	 if(method->method_holder()->klass_part()->is_remote())
	 	{
	 	  if(!thread->is_under_remote_call()){
	         thread->set_remote_call_flag(true);
	         thread->set_remote_call_counter(thread->java_call_counter());
			 gclog_or_tty->print_cr("TransGC method_entry(OSTID:%d TID:%d) %s::%s,thread remote on at call level(%d)!\n",thread->osthread()->thread_id(),((Thread*)thread)->get_pthread_id(),kname->bytes(),name->bytes(),thread->remote_call_counter());
			  tty->print_cr("TransGC method_entry(OSTID:%d TID:%d) %s::%s,thread remote on at call level(%d)!\n",thread->osthread()->thread_id(),((Thread*)thread)->get_pthread_id(),kname->bytes(),name->bytes(),thread->remote_call_counter());

	    }
	    
	 // else
	  	// printf("");//printf("dtrace_method_entry(OSTID:%d TID:%d) %s::%s,thread remote already on!\n",thread->osthread()->thread_id(),((Thread*)thread)->get_pthread_id(),kname->bytes(),name->bytes());

 	} 
 	*/
//else  	
// printf("dtrace_method_entry(OSTID:%d TID:%d) %s::%s\n",thread->osthread()->thread_id(),((Thread*)thread)->get_pthread_id(),kname->bytes(),name->bytes());
	...
	if(DebugTypeGenGC&&PrintAllMethodNames&&thread->is_method_call_tracing())
		{
			...
			char* mn=name->as_C_string();
						...
						//filtering out the common String and Character classes, this also includes all classes beginiing with such names like java/lang/StringBuffer, etc.
				if(!strstr(cn,"java/lang/String")&&!strstr(cn,"java/lang/Character")){
				  tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d! exit!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());				   
				  gclog_or_tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d! exit!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());		
				}
		}
		...
		//the class is registered to be traced.
  		...
  		//  tty->print_cr("TransGC Thread(OSTID:%d) %s::%s(%s),methodcall@%d!\n",thread->osthread()->thread_id(),cn,mn,sign,thread->java_call_counter());				   
			...
			if(strcmp(mn,"await")==0)
				...
				thread->resetPooled();
				thread->reset_thread_counts_in_transaction();
				thread->setTransEdenID(-1);
				if(DebugTypeGenGC){
					tty->print_cr("[TRANS-START-WAKEUP]A new transaction(%d) identified by the pooled thread %d.",thread->transID(),thread->osthread()->thread_id());
					//gclog_or_tty->print_cr("[TRANS-START-WAKEUP]A new transaction(%d) identified by the pooled thread %d.",thread->transID(),thread->osthread()->thread_id());
					}
		      	}
		}
		}
...
#if 0
//by tony 2010-10-4 instrumenting park(),we will monitor park() call in os_linux.cpp.
 //tony-> 2010-07-29 Collecting transEdens
	if(thread->isMonitored()&&thread->hasValidTransID())//by tony 2010-09-21 Only the transaction threads are monitored.
		{
		
//			   if(kname->equals("sun/misc/Unsafe",15)&&name->equals("park",4))
			   if(kname->equals(Arguments::transaction_end_method_string(),Arguments::transaction_end_separator())&&name->equals(Arguments::transaction_end_method_string()+Arguments::transaction_end_separator()+1,strlen(Arguments::transaction_end_method_string())-Arguments::transaction_end_separator()-1))							   
			   	{
										
			   	tty->print_cr("[TRANS-END-PARK] a transaction(%d) is finished by thread %d",thread->transID(), thread->osthread()->thread_id());
				gclog_or_tty->print_cr("[TRANS-END-PARK] a transaction(%d) is finished by thread %d",thread->transID(), thread->osthread()->thread_id());
				/*
				Because we are now in the JRT_LEAF method, we cannot get to safepoint for a GC from here.
				
				We just label the transEden to be collectable for the collection later in the coming allocations. 
				*/
					((GenCollectedHeap*)Universe::heap())->set_transEden_collectable(thread->transEdenID(), thread->transID(),thread->thread_counts_in_transaction());				
					thread->resetTransID();
					thread->setPooled();
 ...
 //<-tony 2010-07-29/
  ...
  //printf("dtrace_method_return(OSTID:%d TID:%d) %s::%s\n",thread->osthread()->thread_id(),((Thread*)thread)->get_pthread_id(),kname->bytes(),name->bytes());
  return 0;
...
// @@PENG Marker support
JRT_LEAF(void, SharedRuntime::markerenter(JavaThread* thread, int mid))		

    pid_t tid = thread->tid();
	intptr_t *sp = thread->last_Java_sp(); // @@PENG
	tty->print_cr("marker %d enter!",mid);


#ifdef TWO_WAY_MARKER
  pid_t tid = thread->tid();
	intptr_t *sp = thread->last_Java_sp(); // @@PENG
	markerStack *st = thread->mkstack(); //markerMap::get(tid);		
	assert (st, "Cannot located the marker stack!");
  
  tty->print_cr("++ m(%d) \t tos(%d)", mid, st->peek() ? st->peek()->desc()->mid() : 0);
  
  // notify predicate watchers
  marker* nm = st->push(mid, tid, sp);
  predicateManager::marker_enter(nm);
//#else
//  predicateManager::marker_enter(mid);
#endif

JRT_END


JRT_LEAF(void, SharedRuntime::markerexit(JavaThread* thread, int mid))

#ifdef TWO_WAY_MARKER
	pid_t tid = thread->tid();
	intptr_t *sp = thread->last_Java_sp(); // @@PENG
	markerStack *st = thread->mkstack(); //markerMap::get(tid);
	assert (st, "Cannot located stack for thread");
  
  if (st->peek() && st->peek()->desc()->mid()!=(short)mid){
    return; //BUG: our instrumentor cannot handle marker regions in 
            // conditionals (if, for, etc.) properly because of the
            // jumping instructions. So we ignore these unbalanced
            // exit for now.
  }

	marker* mk = st->pop();	
	
  tty->print_cr("-- m(%d) \t tos(%d)", mid, mk->desc()->mid());
  
  assert (mk && !mk->desc()->one_way(), "Can't exit an one-way marker.");
	assert (mk && mk->desc()->mid()==(short)mid, "Unbalanced marker-exit");
	
	// notify predicate watchers
	predicateManager::marker_exit(mk);
  delete mk;
#endif
