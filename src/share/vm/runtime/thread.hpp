...
//@@PENG 
class markerStack; 
//#include <vector>  //@@PENG: store per-thread data for clients
...
//didn't use it any way.
//  void set_pthread_id(int pid)  { _pid = pid;  }//added by Tony
//  int  get_pthread_id()  const {  return _pid; }//added by Tony
...
public:
 ...
 protected:
//   int _pid;   //thread id (only applicable for Linux), added by Tony
...
/*tony 07/19/2009 Learning about Threads, Tony*/ 
  int  _call_exits;           //Added by Feng. It is number of method_exits in the thread
  int  _prev_call_exits;
  int  _thread_counts_in_transaction;
  bool _under_remote_call;
  int  _remote_call_counter;
  Thread*  _parent; //to memorize the thread which creates this thread.
  int _transID; //the assigned transaction ID for this thread.
  bool _pooled; // this flag indicate whether curent thread is pooled worker thread or not.
  int  _transEdenID; //a cache indicating which transEden in the remote young this thread should allocate into
  bool _monitor_filter; //by tony 2010-09-16 if this is set,we monitor the method calls made by this thread.
  bool _transStartThread; //indicates if this javaThread is the starting point of the transaction thread, if so, we need to wipe off the transaction area after this thread ends/parks
  bool _method_call_tracing;

  void transCheckBeforeRun();//2010-10-7 tony, we insert our check code in a function body
/*-tony 07/19/2009*/
 //tony-> 2011/02/28 adding new bytecodes
  //@@PENG: to cache the thread id during creation
  pid_t			    _tid;
  markerStack*	_mkstack;
//  std::vector<void*> _client_tag;  
 ...
 //tony-> 2011/02/28 adding new bytecodes
   void init_marker_support(bool main_thread);
  pid_t tid() const 							 { return _tid; }
  markerStack* mkstack()  				 { return _mkstack; }
  /* 
  void set_client_tag(int clientID, void* data) { 
    assert (_client_tag.size() > (size_t)clientID, "out of bound tag access.");
    _client_tag[clientID] = data;
  }
  void* get_client_tag(int clientID) const {
    assert (_client_tag.size() > (size_t)clientID, "out of bound tag access.");
    return _client_tag[clientID];
  }
  */
   //<-tony 2011-02-28
...
/*tony 09/12/2009 Detecting remote objects, Tony*/
 //tony-> 2010-09-16
  bool isMonitored(){return _monitor_filter;}
  void setMonitored(bool b){_monitor_filter=b;}
 //<-tony 2010-09-16
  bool isPooled(){return _pooled;}
  void setPooled(){_pooled=true;}
  void resetPooled(){_pooled=false;}
  Thread* getParentThread(){return _parent;}
  int  transID() 	{return _transID;}
  void setTransID(int id) {_transID=id;}
  bool hasValidTransID(){return _transID!=-1;}
  void resetTransID(){_transID=-1;}

  int transEdenID(){return    _transEdenID;}
  int setTransEdenID(int id){_transEdenID=id;}
  int resetTransEdenID(){   _transEdenID=-1;}

  bool isTransStartThread(){return _transStartThread;}
  void setTransStartThread(){_transStartThread=true;}//no need to reset
  int thread_counts_in_transaction(){return _thread_counts_in_transaction;}//by tony 2010-09-27 investigating the residual of the transactions
  void inc_thread_counts_in_transaction(){_thread_counts_in_transaction++;};
  void reset_thread_counts_in_transaction(){_thread_counts_in_transaction=1;}
  int  java_call_counter()                       { return _java_call_counter; }
  void inc_java_call_counter()                   { _java_call_counter++; }
  void dec_java_call_counter() {
  	    if (_java_call_counter > 0)
      { _java_call_counter--;}
    else
      { tty->print_cr("Invalid nesting of JavaCallWrapper"); }
	    _call_exits++;
//    assert(_java_call_counter > 0, "Invalid nesting of JavaCallWrapper");
//    _java_call_counter--;
  }
  void set_remote_call_flag(bool flag)  { _under_remote_call = flag; }
  bool is_under_remote_call()           { return _under_remote_call; }
  void set_remote_call_counter(int counter) { _remote_call_counter = counter;  }
  int  remote_call_counter()            { return _remote_call_counter;  }
  bool is_method_call_tracing()	{return _method_call_tracing;}
...
/*-tony 09/12/2009*/
 ...
 public:
  ...
  const char* get_thread_name_unsafe() const;
  const char* get_parent_name_unsafe() const;
   /*-tony 01/18/2010 */
