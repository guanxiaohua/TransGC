...
/*tony 09/08/2009 Simulating DTrace Enablings, Tony*/ 
#define DTRACE_ENABLED //by tony 2010-10-7 No need for it now!
/*-tony 09/08/2009*/
...
#define RUNTIME_FLAGS(develop, develop_pd, product, product_pd, diagnostic, experimental, notproduct, manageable, product_rw, lp64_product) \
  ...
  develop(bool, DieOnSafepointTimeout, false,                               \
          ...
          \
product(bool, ForceSlowPath,  false, "force everything goes to            \
                                               slow path")                  \
\
product(bool, TransGC,  false, "enable the trasaction based GC functionality")\
																			\
product(bool, Pretenure,  false, "enable pretenuring when training is done")\
																			\
product(bool, PretenureToOld,  true, "pretenure to old gen, false means to local young.")\
																			\
product(bool, ExamingAllocationSite,  false, "check the allcoation sites of the residuals")\
																			\
product(bool, CountChildrenForTrans,  false, "allocate the children threads into transaction area.")\
														\
product(bool, UseParkUnpark,false, "if true, use park() & unpark() for detecting transactions, otherwise, use await()")\
																			\
product(bool,  PrintRemoteMethodNames,false, "trace the first remote method calls")\
																			\
product(bool,  ExamineResiduals,false, "print out the types of resuduals for transEdens. ")\
\
product(bool, OnlyTagObjInTrans,false, "if true, for JVMTI service, only tag those objects allocated into transNewGen.")\
																			\
product(bool,  PrintAllMethodNames,false, "trace all the method calls")\
																			\
  ...
  product_pd(intx, NewRatio,                                                \
          "Ratio of new/old generation sizes")                              \
                                                                            \
  /* Type-based Generational GC. Added by Feng. Feb, 2006 */                \
  product(bool, UseTypeGenGC, false, "Use type-based GC")                   \
																			\
  product(bool, NoPretenuringBytes,false, /*by tony 2010-02-11*/                \
          "if true, NoPretenuringBytes ")                                   \
		\
  product(bool, ResidualDemographics,false, /*by tony 2010-01-16 printout the demographics of residuals*/                \
          "if true, we will print out the residual's demographics for each transGC ")                                   \
		\
  product(bool, ThreadRunAsTransaction,false, /*by tony 2010-01-19 regard the thread run as a signal of transaction start or not?*/                \
          "if true, regard the thread run as a signal of transaction star, otherwise, only park()/unpark() will be honored.")                                   \
		\
  product(bool, CheckResidualSource,false, /*by tony 2010-01-21 */                \
          "check the type of the object that is holding the residual object from garbage collection.")                                   \
		\
  product(intx, MAX_TRANSEDEN,   5, /*Added by tony,2010-7-26*/                \
          "The maximum number of TransEdens in the remote young gen.")                                   \
          \
  product(intx, TRAINING_AFTER_GC,   5, /*by tony 2010-11-04 analysing the residuals*/                \
          "The residual training will not start until the transgc count exceeds TRAINING_AFTER_GC.")                                   \
          \
  product(bool, APPLY_WHILE_TRAINING,false, /*by tony 2010-11-09 applying the residual training result*/                \
          "if true, we will begin applying the training result as soon as it becomes available, if alse, we will wait for the training is done. ")                                   \
		\
  product(bool, STILL_CURIOUS,false, /*by tony 2010-11-09 applying the residual training result*/                \
          "if true, we will examine the leftover of the trained transEden ")                                   \
		\
  product(bool, AVOID_COLLECT_TRAINED,false, /*by tony 2010-11-09 applying the residual training result*/                \
          "if true, we will not collect the trained transaction area, instead, just reset the top() pointer, if false, collect it as usual.")                                   \
		\
  product(intx, UNCHANGED_TIME_FOR_TRAINING_END,   3, /*by tony 2010-11-04 analysing the residuals*/                \
          "if the training result doesn't change for consecutively UNCHANGED_TIME_FOR_TRAINING_END times, then the training is over.")                                   \
          \
  product(bool, DirtAllCardsEachGC, false, "just a debug switch, if true, before and after gc, veriry() will drit all cards before verifying rememberset, must be combined with  VerifyBeforeGC or VerifyAfterGC.")                   \
  \
  product(bool, NormalGCInvoke, true, "Invoke GC when heap is full")        \
									    \
  product(bool, MinorGCGoingOn, false, "When the minor GC is going on, this if set to be true.")        \
									    \
  product(intx, GCFreq,   5*K, /*Added by Feng. April, 2004*/                \
          "The frequency of invoking GC")                                   \
  product(bool, DebugTypeGenGC, false,                                      \
          "Print general debugging information")                            \
          \
  product(bool, NeedTransGCInfo, false,                                      \
          "Print TransGC info on GCs")                            \
          \
  product(intx, RemoteRatio, 1,/*Added by Feng. Feb, 2006, copied by tony 2009*/              \
          "Ratio of remote/local generation sizes")                         \
																		    \
  product(bool, reverseRemoteRatio, false, /*Added by Feng. Feb, 2006 copied by tony 2009*/       \
           "flag to identify RemoteRatio is remote/local or local/remote")  \
																	           \
