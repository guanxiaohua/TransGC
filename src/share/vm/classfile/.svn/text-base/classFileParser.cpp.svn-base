...
#define MARKER_PRED_MAGIC                 0xBABECAFE // PENG DU
#define IS_MAGIC(i)												(MARKER_PRED_MAGIC == (i))
#define MARKER_CLASSID()                	(pred_idx_base() + 1)
#define MARKER_FIRST()               			(pred_idx_base() + 1)
 //<-tony 2011-02-28
      ...
      case JVM_CONSTANT_Integer :
        {
          ...
          cp->int_at_put(index, (jint) bytes);
		   ...
		   //tony-> 2011/02/28 adding new bytecodes
          /* check if num is our magic number */
          juint num = (juint)cp->int_at(index); // PENG DU
          if (IS_MAGIC(num)) set_pred_idx_base(index); // skip the byte
		    //<-tony 2011-02-28
        }
        break;
...
//@@PENG
// This is the place where the embedded marker predicates are parsed.
// 
// Format: 
// +---------------+
// |     const1    |
...
// +---------------+
// |    marker #1  |    <---+--->
// +---------------+        |  marker (integer) & its predicates (string)
// |    pred #1    |        |
// +---------------+        |
// |    marker #2  |        |
// +---------------+        |
...
// +---------------+
// 
void ClassFileParser::parse_marker_descs(constantPoolHandle cp, 
	int length, symbolHandle name, TRAPS) {
	bool has_markers = pred_idx_base();	
	if (has_markers){ // a threading class processed by our marker-gen tool		
		#define copy_str(src, dst) \
			dst = NEW_C_HEAP_ARRAY(char, strlen(src)); \
			strcpy(dst, src);
			
		{
			...
			buf = name->as_C_string();
			tty->print_cr("Name:%s",buf);
			copy_str(buf, cid);
			
			// DEBUG_ONLY(tty->print_cr("[AS] Class %s -- %d", cid, MARKER_FIRST()));
			
			// process every predicate in this thread
			for (int i=MARKER_FIRST(); i<length; i+=2){
				u2 mid;
				short oneway = 1;//by tony, defaultly oneway
				...
				tty->print_cr("i:%d,markid:%s",i,buf);
				...
				mid = atoi(buf);
*/
			//the marker id should be an int type
				mid = cp->int_at(i);
				// parse predicates
				symbolOop sym = cp->symbol_at(i+1);
				buf = sym->as_C_string();

				tty->print_cr("i+1:%d,predicates:%s",i+1,buf);
			
				copy_str(buf, preds);
				
				markerDescriptor* md;
        // mid == 0 is also a legitimate marker for us
        
				//if (mid != 0) // regular marker
        md = markerDescManager::add(mid, cid, preds, oneway);
				//else          // psuedo marker
				//	md = new markerDescriptor(mid, cid, preds, oneway);
				
        predicateManager::broadcast(md);
        
        tty->print_cr("\t<%d%s> %s", mid, oneway ? "@" : "", preds);
        
        #ifdef PROFILE_APP_METHOD
        if (strstr(preds, "appCls") != NULL) {
          KlassHandle k(cp->pool_holder());
          k->set_is_app_klass(1);
          //tty->print_cr("AppClass: %s", cid);
        }
        #endif
			}
		}
	...
	//Klass* klaz = cp->pool_holder()->klass_part();
	//assert (klaz, "[AS] klass is not properly initialized.");	
	//klaz->set_has_code_markers(has_markers);
	//klaz->set_need_x2c_adapter(false); // no x2c adapter assumed
	
	DEBUG_ONLY(
	  //if (has_markers)
		//tty->print_cr("[AS] Class %s has code markers", name->as_C_string());
	)
}
...
methodHandle ClassFileParser::parse_method(constantPoolHandle cp, bool is_interface,
 ...
 //tony-> 2011/02/28 adding new bytecodes
  m->set_has_markers(false); // @@PENG: no marker assumed
 //<-tony 2011-02-28
...
instanceKlassHandle ClassFileParser::parseClassFile(symbolHandle name,
  ...
  if (pred_idx_base()){ //@@PENG: there are markers in this class		
    // parse the marker information from constant pool
    parse_marker_descs(cp, cp_size, _class_name, CHECK_(nullHandle));		
    // notify predicates about the loaded methods
    // predicateManager::methods_loaded(this_klass->methods());		
    // reset the index
    set_pred_idx_base(0);
  }
