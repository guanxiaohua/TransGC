...
//	klassOop    SystemDictionary::_remote_klass               =  NULL;  //Added by Feng at Jan 27
//this is done by template()
 /*-tony 09/12/2009*/
...
/*
  ...
  if (SystemDictionary::remote_klass_loaded()) {  //Added by Feng
    if (k->is_subtype_of(SystemDictionary::remote_klass()))
      { 
        remotable = 1;
        ik->set_remote_flag(true);
      }
  }
  */
	 //tony-> 2010-11-22 adding the _is_traced flag in Klass, indicating to the run time tracer whether or not to trace method call of this class.
	 if(TransGC)
	 	{
	 	
	 		ik->set_residual_flag(false); //by tony 2010-11-04 analysing the residuals, initially, each class is labeled as non-residual
			ik->set_traced(false);
			//by tony 2011/01/13 check the residuals.
			ik->set_residual_counts(0);

				ResourceMark rm;
				//org.apache.tomcat.util.net.MasterSlaveWorkerThread
			if(!UseParkUnpark)//by tony 2010-12-16 Recovering Park/Unpark detection.
			{
				
			if(!Universe::isFoundTransClass())
				{
				if(strcmp(ik->external_name(),"org.apache.tomcat.util.net.MasterSlaveWorkerThread")==0)
				 	{
						ik->set_traced(true);
						tty->print_cr("[TRANSGC]Class org/apache/tomcat/util/net/MasterSlaveWorkerThread marked for method tracing!");
						Universe::setFoundTransClass();
...
/*	
  if (SystemDictionary::runnable_klass_loaded()) {  //Added by Tony
    if (k->is_subtype_of(SystemDictionary::runnable_klass()))
      { 
        ik->set_runnable_flag(true);
      }
  }
	*/
  /*
  if(ProfileObj) {
    if (k()->klass_part()->oop_is_array()){
       tty->print_cr("CL " INTPTR_FORMAT " [%s] %s", Klass::cast(k())->java_mirror(), ik->external_name(),remotable?"Remote":"Local");
    }
    else{
       tty->print_cr("CL " INTPTR_FORMAT " [%s] %s", ik->java_mirror(), ik->external_name(),remotable?"Remote":"Local");
    }
    ...
    if(ProfileMethod){
      if (!k()->klass_part()->oop_is_array()){
        ...
        int num_methods = methods->length();
        int i_meth=0;
        for (; i_meth<num_methods; i_meth++) {
           methodOop m= (methodOop)methods->obj_at(i_meth);
           if (k()->klass_part()->oop_is_array())
              tty->print_cr("ML %p %s %p", (jmethodID)m->jni_id(), m->name()->as_C_string(), Klass::cast(k())->java_mirror());
           else
              tty->print_cr("ML %p %s %p", (jmethodID)m->jni_id(), m->name()->as_C_string(), ik->java_mirror());
        }
      }
    }//if ProfileMethod
  } //end if ProfileObj
...
bool SystemDictionary::initialize_wk_klass(WKID id, int init_opt, TRAPS) {
  ...
  if (init_opt == SystemDictionary::Opt_Kernel) {
	...
	/*tony 09/12/2009 Detecting remote objects, Tony*/ 
	 int remoteID=WK_KLASS_ENUM_NAME(remote_klass);
//	 int runnableID=WK_KLASS_ENUM_NAME(runnable_klass);
	 int threadID=WK_KLASS_ENUM_NAME(thread_klass);
	if(id==remoteID)
		tty->print_cr("loading java_rmi_Remote...id=%d",(int)id);
/*
	else if(id==runnableID)
		tty->print_cr("loading java_lang_Runnable...id=%d",(int)id);
/
	
	/*-tony 09/12/2009*/

 /*tony 09/12/2009 Detecting remote objects, Tony*/ 
	if(id==remoteID)//||id==runnableID)
		{//force jvm to load the rmi class
		try_load=true;
		must_load=true;
		}
 ...
 /*-tony 09/12/2009*/
  ...
  if ((*klassp) == NULL && try_load) {
	...
	if(id==remoteID)//||id==runnableID)
			{
			if((*klassp) != NULL)
				tty->print_cr("the transGC required class loaded successful...id=%d",(int)id);
			else
				tty->print_cr("the transGC required class loading failed!...id=%d",(int)id);
			}
	/*-tony 09/12/2009*/
  return ((*klassp) != NULL);
}
