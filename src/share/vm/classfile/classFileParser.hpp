...
class ClassFileParser VALUE_OBJ_CLASS_SPEC {
 ...
 //tony-> 2011/02/28 adding new bytecodes
  int  _pred_idx_base; // PENG DU
  int pred_idx_base() const 			{ return _pred_idx_base; }
  void set_pred_idx_base(int value) 	{ _pred_idx_base = value; }
  void parse_marker_descs(constantPoolHandle cp, 
																	int length, 
																	symbolHandle name, 
																	TRAPS);
  //<-tony 2011-02-28
 ...
 public:
  // Constructor
  ClassFileParser(ClassFileStream* st):_pred_idx_base(0) { set_stream(st); 
 // set_pred_idx_base(0);
  ;//by tony 2011/02/28
  }
