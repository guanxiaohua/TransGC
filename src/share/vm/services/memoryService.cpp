...
if(TransGC)
{
 guarantee(heap->n_gens() == 3, "Only support three-generation heap when TransGC is enabled");
 ...
 /*-tony 01/18/2010 */
  ...
  if (two_gen_policy != NULL) {
    ...
    switch (kind) {
	 ...
	 /*tony 01/18/2010 Porting Type Based GC, Tony*/ 	 
	if(TransGC&&policy->is_typegen_policy())
		{
...
if(TransGC)
{
	 add_generation_memory_pool(heap->get_gen(2), _major_gc_manager);
}else
...
TraceMemoryManagerStats::TraceMemoryManagerStats(Generation::Name kind) {
  switch (kind) {
    case Generation::DefNew:
	case Generation::TransNew: //2010-7-15 added by tony guan
#ifndef SERIALGC
    case Generation::ParNew:
    case Generation::ASParNew:
#endif // SERIALGC
