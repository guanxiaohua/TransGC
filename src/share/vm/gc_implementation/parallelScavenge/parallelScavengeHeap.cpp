...
HeapWord* ParallelScavengeHeap::mem_allocate_with_pretenuring(size_t size, bool is_tlab, bool pretenuring) {
  assert(!SafepointSynchronize::is_at_safepoint(), "should not be at safepoint");
  assert(Thread::current() != (Thread*)VMThread::vm_thread(), "should not be in vm thread");
  assert(!Heap_lock->owned_by_self(), "this thread should not own the Heap_lock");

   bool isGCLimitExceeded=false;
  if(!pretenuring) 
  	 return mem_allocate(size,false, is_tlab,&isGCLimitExceeded);
  ...
  do {
   //allocate the object directly in the mature space  
    MutexLocker ml(Heap_lock);
    result = old_gen()->allocate(size, is_tlab);
    
    if (result == NULL) {
      // Generate a VM operation
      VM_ParallelGCFailedAllocation op(size, is_tlab,999);//FixME: 999 was added by tony just for the time being.
      VMThread::execute(&op);
