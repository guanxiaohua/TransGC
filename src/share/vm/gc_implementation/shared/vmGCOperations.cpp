...
void VM_TypeGenCollectForAllocation::doit() {
  JvmtiGCForAllocationMarker jgcm;
  notify_gc_begin(true);
  GenCollectedHeap* gch = GenCollectedHeap::heap();
  _res = gch->satisfy_failed_type_based_allocation(_size, _tlab, _remote);
...
void VM_CollectTransEden::doit() {
	  notify_gc_begin(true);
	  GenCollectedHeap* gch = GenCollectedHeap::heap();

	  gch->collectTransEden(_transEdenID);
	  //_res = gch->satisfy_failed_type_based_allocation(_size, _tlab, _remote);
	  notify_gc_end();
	} 
