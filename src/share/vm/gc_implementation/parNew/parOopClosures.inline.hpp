...
// Since I modified the oopClosure to contain Generation* instead of DefNewGeneration, I have to modify this 
// file too to get it compiled.
// the modification is:(ParNewGeneration*)_g
 /*-tony 2010-06-14 */
...
template <class T> inline void ParScanWeakRefClosure::do_oop_work(T* p) {
  ...
  if ((HeapWord*)obj < _boundary && !((ParNewGeneration*)_g)->to()->is_in_reserved(obj)) {
...
template <class T> inline void ParScanClosure::par_do_barrier(T* p) {
  ...
  assert(false,"ParScanClosure::par_do_barrier gets called, trace!");
  oop obj = oopDesc::load_decode_heap_oop_not_null(p);
...
inline void ParScanClosure::do_oop_work(T* p,
  ...
  if (!oopDesc::is_null(heap_oop)) {
    ...
    if ((HeapWord*)obj < _boundary) {
      assert(!((ParNewGeneration*)_g)->to()->is_in_reserved(obj), "Scanning field twice?");
      ...
      if (m->is_marked()) { // Contains forwarding pointer.
        ...
        size_t obj_sz = obj->size_given_klass(objK->klass_part());
        new_obj = ((ParNewGeneration*)_g)->copy_to_survivor_space(_par_scan_state, obj, obj_sz, m);
        oopDesc::encode_store_heap_oop_not_null(p, new_obj);
        if (root_scan) {
