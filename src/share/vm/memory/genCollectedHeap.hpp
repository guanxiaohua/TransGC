...
class GenCollectedHeap : public SharedHeap {
  ...
  friend class TypeGenCollectorPolicy;  //Added by Feng. Feb,2006 //stolen by tony, 2010-1-17
  friend class CMSCollector;
  friend class GenMarkSweep;
  friend class VM_GenCollectForAllocation;
  friend class VM_GenCollectForPermanentAllocation;
  friend class VM_TypeGenCollectForAllocation;   //Added by Feng. Feb, 2006 //stolen by tony, 2010-1-17
  ...
  friend class  VM_CollectTransEden; ////by tony 2010-07-29
  	/*tony 01/18/2010 Porting Type Based GC, Tony*/ 
friend class TwoGenerationCollectorPolicy;
  /*-tony 01/18/2010 */

  /*tony 2010-06-14 TransGCDefNew, Tony*/ 
friend class TransNewGeneration;
  /*-tony 2010-06-14 */
...
public:
  enum SomeConstants {
   ...
   void invoke_transEden_collection(int transEdenID);
    //<-tony 2010-07-29/
...
protected:
 ...
 //
 //The following three 'type_based' methods are used in type-based GC. Added by Feng, Feb 2006.
 //
 HeapWord* attempt_type_based_allocation(size_t size,
//                               bool   is_large_noref,
                               bool   is_tlab,
                               bool   first_only,
                               bool   remote);
  ...
  // Helper function for two callbacks below.
  // Considers collection of the first max_level+1 generations.
  void type_based_resize_young();
    

  /*tony 01/18/2010 Porting Type Based GC, Tony*/ 
  void do_type_based_collection(bool  full,
                                     bool   clear_all_soft_refs,
                                     size_t size,
                                     bool   is_tlab,
                                     int    max_level,
  ...
  // This function does everything necessary/possible to satisfy an
  // allocation request that failed in the youngest generation that should
  // have handled it (including collection, expansion, etc.)  Sets
  // "notify_ref_lock" to true if it enqueues any weak references.
...
//                                         bool is_large_noref,
                                         bool is_tlab,
                                         bool pretenuring); //Added by Feng, Jan 2006
 /*-tony 11/16/2009*/
 
  //tony-> 2010-08-19 Collecting transEdens
 virtual int get_trans_eden_id(void* add);
 //<-tony 2010-08-19/
  ...
  Generation* prev_gen(Generation* gen) const {
    ...
    if (l == 0) return NULL;
	 //tony-> 2010-08-19 Collecting transEdens
		...
		return _gens[n_gens()-1]; //this for perm
	  //<-tony 2010-08-19/
