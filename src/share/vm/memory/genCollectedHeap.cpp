...
void GenCollectedHeap::post_initialize() {
 ...
 /*tony 01/18/2010 Porting Type Based GC, Tony*/ 
	 Generation* old_gen = get_gen(TransGC?2:1);
 /*-tony 01/18/2010 */
 /* 
...
//Added by Feng at Feb, 2006
HeapWord* GenCollectedHeap::attempt_type_based_allocation(size_t size,
					    //   bool is_large_noref,
                                               bool is_tlab,
					       bool first_only,
					       bool remote) {

  assert(collector_policy()->is_typegen_policy(), "Not type-based collector.");
  HeapWord* res;


  if (!remote)  {
  	//allocate local objects in gen[0]
       if (_gens[0]->should_allocate(size, is_tlab)) {
         res = _gens[0]->allocate(size, is_tlab);
         if (res != NULL){ 
// 	    		if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]:attempt_type_based_allocation remote?%d allocated in gen[0], size:%d",remote,size);}
				return res;
				}
          else if (first_only) return NULL;
      }      
  }
  else{
  	//allocate remote objects in gen[1]
  	if (_gens[1]->should_allocate(size,  is_tlab)) {
         res = _gens[1]->allocate(size, is_tlab);
		 
         if (res != NULL) {
	//     	if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]:attempt_type_based_allocation remote?%d allocated in gen[1], size:%d",remote,size);}
		 	return res;
  ...
  for (int i = 2; i < _n_gens; i++) {
    if (_gens[i]->should_allocate(size,  is_tlab)) {
      res = _gens[i]->allocate(size,  is_tlab);

      if (res != NULL) {
	  //if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]:attempt_type_based_allocation remote?%d allocated in gen[%d], size:%d",remote,i,size);}
		return res;}
      else if (first_only) break;
    }
  }
...
HeapWord* GenCollectedHeap::mem_allocate_with_pretenuring(size_t size,
                                        // bool is_large_noref,
                                         bool is_tlab,
                                         bool pretenuring) {
  
      return collector_policy()->mem_allocate_with_pretenuring(size,is_tlab, pretenuring);
}

//Added by Feng. Sep-17-2006
void GenCollectedHeap::type_based_resize_young() {

 /*tony 01/18/2010 Porting Type Based GC, Tony*/ 

 //don't do the resize here!
 ...
 /*
  ...
  int min_alignment = (uintx) Generation::GenGrain;
  int min_young_size = 3* alignment; //the minimum size of a survivor space or eden space is car_size()=2^16. A typical young gen consisting of one eden plus two survivor spaces has the size equal 3*car_size() 
  double min_size_ratio, max_size_ratio;

  double size_ratio = Universe::space_ratio();

  size_t  init_young_size = _gen_specs[0]->init_size() + _gen_specs[1]->init_size();
  size_t max_young_size = _gen_specs[0]->max_size() + _gen_specs[1]->max_size();

  //Calculate the lower-bound and upper-bound of size_ratio. Reset the size_ratio if necessary.
  min_size_ratio = (double)min_young_size/(init_young_size - min_young_size);
  max_size_ratio = (init_young_size - min_young_size)/(double)min_young_size;

  if(size_ratio < min_size_ratio)
	{ size_ratio = min_size_ratio; }
  else if(size_ratio > max_size_ratio)
	{ size_ratio = max_size_ratio; }
  Universe::set_space_ratio(size_ratio);

  size_t _init_s_gen_size = align_size_down(round(init_young_size*size_ratio /(size_ratio+1)),
                                            min_alignment);
  size_t  _max_s_gen_size = align_size_down(round(max_young_size*size_ratio /(size_ratio+1)),
                                            min_alignment); 
  size_t _init_o_gen_size = init_young_size -  _init_s_gen_size;
  size_t _max_o_gen_size  = max_young_size  -  _max_s_gen_size;

      
  // Make sure the sizes are all aligned.
  _gen_specs[0]->set_init_size(align_size_up(_init_s_gen_size, alignment));
  _gen_specs[0]->set_max_size(align_size_up(_max_s_gen_size, alignment));
  _gen_specs[1]->set_init_size(align_size_up(_init_o_gen_size, alignment));
  _gen_specs[1]->set_max_size(align_size_up(_max_o_gen_size, alignment));

 // ReservedSpace heap_rs((char *)_reserved.start(), _reserved.byte_size());
  ReservedSpace heap_rs = *_heap_rs;
   
  //re-initialize all defnew generations, free the old ones

  for (i = 0; i < _n_gens-1; i++) {
    ((DefNewGeneration *)_gens[i])->release_gen();
    Universe::heap()->barrier_set()->uncommit_covered_region(i);
  }

  for (i = 0; i < _n_gens-1; i++) {
    ReservedSpace this_rs = heap_rs.first_part(_gen_specs[i]->max_size());
    ((DefNewGeneration *)_gens[i])->reinitialize_gen(this_rs, _gen_specs[i]->max_size());
    heap_rs = heap_rs.last_part(_gen_specs[i]->max_size());
  }
 ...
 //tony-> 2010-07-29 Collecting transEdens
	void GenCollectedHeap::collectTransEden(int id)
 	{
 	//we ignore id, just collect one transEden at a time.
	   	Generation* gen=get_gen(1);	
		if(gen->kind()!=Generation::TransNew)
			return;

		
		 //tony-> 2010-08-12 Collecting transEdens
...

  {
    ...
    const char* gc_cause_str = "GC-Transaction Edens";
    ...
    for (int i = 1; i <= 1; i++) {
        ...
        if (PrintGC && Verbose) {
          gclog_or_tty->print_cr("[TransGC]level=%d invoke=%d",
                     i,
                     _gens[i]->stat_record()->invocations);
        }
...
//        if (VerifyBeforeGC && i >= VerifyGCLevel &&total_collections() >= VerifyGCStartAt) 
        ...
        // Do collection work
        {
          ...
          if (rp->discovery_is_atomic()) {
			...
			//scan for the first collectable thransEden
		  for(int j=0;j<MAX_TRANSEDEN;j++){
		  	if(_gens[i]->as_TransNewGeneration()->getTransEden(j)->state==TransEden::Collectable)
				{
				id=j;
				break;
		  		}
		  	}
		  	tty->print_cr("[TransGC]collecting TransEden-%d",id);		  
		  	gclog_or_tty->print_cr("[TransGC]collecting TransEden-%d",id);

			_gens[i]->as_TransNewGeneration()->collectTransEden(id);
...
//          _gens[i]->collect(full, clear_all_soft_refs, size, is_tlab);
  ...
  if (PrintHeapAtGC) {
   ...
   void GenCollectedHeap::set_transEden_collectable(int id,int transid,int threadcount)
   	{
	   	Generation* gen=get_gen(1);	
		if(gen->kind()!=Generation::TransNew)
			return;
		gen->as_TransNewGeneration()->set_transEden_collectable(id,transid,threadcount);
   	}

 
   void GenCollectedHeap::invoke_transEden_collection(int transEdenID)
   	{
     VM_CollectTransEden op(Universe::heap()->total_collections(),transEdenID);
	 VMThread::execute(&op);
...
void GenCollectedHeap::do_type_based_collection(bool  full,
 ...
 //performance analysis will need this info
 //if(DebugTypeGenGC)
// if(NeedTransGCInfo)
 	{gclog_or_tty->print_cr("[TONY]do_type_based_collection: remote:%d,is_tlab:%d,size:%d",remote,is_tlab,size);}
...

  {
    ...
    for (int i = starting_level; i <= max_level; i++) {
		 ...
		 /*tony 01/18/2010 Porting Type Based GC, Tony*/ 
	    bool should_collect_b = false;
	    if (i==0)  should_collect_b = !remote;
	    else if (i==1)  should_collect_b = remote;
	    else
          should_collect_b = _gens[i]->should_collect(full, size, is_tlab);
      ...
      //if (_gens[i]->should_collect(full, size, is_tlab)) {
        ...
        // Do collection work
        {
		  ...
		  if(DebugTypeGenGC&&i==1){
		  	gclog_or_tty->print_cr("collecting remote young!");
		  	}
    ...
    if (complete) { // We did a "major" collection
	 ...
	 //tony-> 2010-07-19 Collecting TransNewGen Tony
		  if(DebugTypeGenGC){
		  	gclog_or_tty->print_cr("We are now resetting the transEdens if it's empty after this full gc.");
     		 get_gen(1)->as_TransNewGeneration()->reset_transEdens();	
  	  	}
      //<-tony 2010-07-19/
 ...
 gclog_or_tty->print_cr("[TransGC-GC]:we are disabling the compute_new_size after a full GC. for debug use.");
...
//here we can be allocating some tlabs, so don't let it slip away
...
void GenCollectedHeap::do_collection(bool  full,
   ...
   //ugly code. In case some methods call do_collection
  if(collector_policy()->is_typegen_policy()){
    do_type_based_collection(full, clear_all_soft_refs, size, is_tlab, max_level, false);
    return;
  }
   /*-tony 01/18/2010 */
...
/*
 if(DebugTypeGenGC&&level<=1){
 	
 	gclog_or_tty->print_cr("gen_process_strong_roots 1 verifying remset...");
	rem_set()->verify();
	gclog_or_tty->print_cr("[TONY] remset verified");
 ...
 */
  if (younger_gens_as_roots) {
    if (!_gen_process_strong_tasks->is_task_claimed(GCH_PS_younger_gens)) {
     ...
     //Modified by Feng, we treat genp[0] as older gen as well. tricky!!!
     if(UseTypeGenGC){
         for (int i = 0; i < level; i++) {

//			 if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY] older_gens->set_generation(_gens[%d])",i);}
			 
             older_gens->set_generation(_gens[i]);
             //rem_set()->younger_refs_iterate(_gens[i], older_gens);
           //  _gens[i]->oop_iterate(older_gens);        
             _gens[i]->oop_iterate(older_gens);        
/*
			 if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY] _gens[%d]->oop_iterate(older_gens) done!",i);
			 	gclog_or_tty->print_cr("verifying remset...");
				rem_set()->verify();
				gclog_or_tty->print_cr("[TONY] remset verified");
	...
	*/		 
		 ...
		 ////by tony 2010-08-23  
		 		  //Here, we should also treat the 2nd level differenetly.
		 		  //we need to scan the other edens

	 if(level==1){
	// 	if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]Now we treat on other transEdens.");}
             older_gens->set_generation(_gens[level]);
             //rem_set()->younger_refs_iterate(_gens[i], older_gens);
//             _gens[level]->oop_iterate(older_gens);
            _gens[level]->oop_iterate_with_some_excluded(older_gens);
/*
			if(DebugTypeGenGC){
			  	gclog_or_tty->print_cr("gen(1)oop_iterate_with_some_excluded done,verifying remset...");
				rem_set()->verify();
				gclog_or_tty->print_cr("[TONY] remset verified");
				}
*/
  ...
  if(UseTypeGenGC && level==0){
    older_gens->set_generation(_gens[1]);
    _gens[1]->oop_iterate(older_gens);
    //rem_set()->younger_refs_iterate(_gens[1], older_gens);
    older_gens->reset_generation();
    older_gen_level = 2;
	/*
		if(DebugTypeGenGC){
			  	gclog_or_tty->print_cr("gen(1)oop_iterate done,verifying remset...");
				rem_set()->verify();
				gclog_or_tty->print_cr("[TONY] remset verified");
				}
*/
  ...
  // older-gen scanning.
  for (int i = older_gen_level; i < _n_gens; i++) { //changed from level+1 to older_gen_level //by tony 2010-2-15
    older_gens->set_generation(_gens[i]);
    rem_set()->younger_refs_iterate(_gens[i], older_gens);
    older_gens->reset_generation();
...
/*
			if(DebugTypeGenGC){
			  	gclog_or_tty->print_cr("gen(%d)younger_refs_iterate done,verifying remset...",i);
				rem_set()->verify();
				gclog_or_tty->print_cr("[TONY] remset verified");
				}
				*/
...
oop_since_save_marks_iterate(int level,                                 \
  ...
  if(UseTypeGenGC){                                                     \
    _gens[0]->oop_since_save_marks_iterate##nv_suffix(older);           \
    if(level==1)                                                        \
     {  _gens[1]->oop_since_save_marks_iterate##nv_suffix(older); } \
  }                                                                     \
  else                                                                  \
...
bool GenCollectedHeap::no_allocs_since_save_marks(int level) {
 ...
 /*tony 02/06/2010 Fixing Type Based GC, Tony*/ 
  if(UseTypeGenGC && level==1){
   //do an extra check on gen[0] if we are using type gc
     if (!_gens[0]->no_allocs_since_save_marks()) return false;
  }
...
void GenCollectedHeap::object_iterate_since_last_GC(ObjectClosure* cl) {
  for (int i = 0; i < _n_gens; i++) {
 // 	if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]object_iterate_since_last_GC in gen(%d)...",i);}
...
HeapWord* GenCollectedHeap::block_start(const void* addr) const {
	if(!is_in_reserved(addr))
		tty->print_cr("block_start of address outside of heap"PTR_FORMAT,addr);
...
//  gclog_or_tty->print_cr("GenCollectedHeap::allocate_new_tlab calling mem_allocate");//by tony
...
void GenCollectedHeap::verify(bool allow_dirty, bool silent) {
  ...
  if(DirtAllCardsEachGC)
  	{
  //tony-> 2010-08-18 Collecting transEdens
  //invalidate cardtables
	 GenRemSet* rs = rem_set();
    // Invalidate the cards corresponding to the currently used
    // region and clear those corresponding to the evacuated region
    // of all generations just collected (i.e. level and younger).
    rs->invalidate_or_clear(perm_gen(),
                            true /* younger */,
                            true /* perm */);
	//then the verify should be OK.
 //<-tony 2010-08-18/
	gclog_or_tty->print_cr("[DEBUG]invalidate cardtables before and after GCs, just for debugging, should be removed later.");
...
void GenCollectedHeap::gc_prologue(bool full) {
 ...
 /*tony 01/18/2010 Porting Type Based GC, Tony*/ 
  ...
  if (PrintGC && Verbose && UseTypeGenGC) {
        gclog_or_tty->print_cr("[Total_remote_objects: %d (%lld bytes), Total_local_objects: %d (%lld bytes)]", 
        Universe::remote_obj_count(),Universe::remote_obj_bytes(), 
        Universe::local_obj_count(), Universe::local_obj_bytes());  //Feng
  	}
 /*-tony 01/18/2010 */
  ...
  if(!TransGC||gc_cause()==GCCause::_allocation_profiler)
...
HeapWord* GenCollectedHeap::satisfy_failed_type_based_allocation(size_t size,
//                                                      bool   is_large_noref,
                                                      bool   is_tlab,
  //                                                    bool&  notify_ref_lock,
                                                      bool remote) {
  if( collector_policy()->is_typegen_policy() )
    return ((TypeGenCollectorPolicy*)(collector_policy()))->satisfy_failed_type_based_allocation(size,
                                                      // is_large_noref,
                                                       is_tlab,
                                                       //notify_ref_lock,
                                                       remote);
...
//Feng. Feb, 2006
int GenCollectedHeap::addr_to_gen_id(void* addr) {
  int i;
  for (i = 0; i < _n_gens; i++) {
    if( _gens[i]->is_in_reserved(addr) )
        return i;
  }
  if (perm_gen()->is_in_reserved(addr)) return i;
  // should not reach here
  // Delete fatal checking for rootset
  // fatal("GenCollectedHeap::addr_to_gen_id: out of any generations");
  return -1;
}
 /*-tony 01/18/2010 */
  //tony-> 2010-08-19 Collecting transEdens
  int GenCollectedHeap::get_trans_eden_id(void* add){
 	if(!TransGC)
		return -1;
     return		_gens[1]->as_TransNewGeneration()->get_trans_eden_id(add);
