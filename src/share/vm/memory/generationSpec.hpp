...
class TransGenerationSpec :public GenerationSpec{
  public:
   TransGenerationSpec(Generation::Name name, size_t init_size, size_t max_size):GenerationSpec(name,init_size,max_size) {}
  // Return the number of regions contained in the generation which
  // might need to be independently covered by a remembered set.
  virtual int n_covered_regions() const { return MAX_TRANSEDEN+1; }//MAX_TRANSEDEN and 1 Original Eden, will remove t he original Eden once I am sure that everything is OK, for now, live  with it. //by tony 2010-07-04
//  virtual int n_covered_regions() const { return 1; }//MAX_TRANSEDEN and 1 Original Eden, will remove t he original Eden once I am sure that everything is OK, for now, live  with it. //by tony 2010-07-04
 };
 //<-tony 2010-07-04/
