...
private:
  ...
  //  assert(!MinorGCGoingOn,"ClearNoncleanCardWrapper::clear_card() called during minor GC, trace!");
...
public:
  ...
  void do_MemRegion(MemRegion mr) {
	...
	int genid=Universe::heap()->addr_to_gen_id(mr.start());
//	gclog_or_tty->print_cr("[TransGC-GC]return ClearNonCleanCardWrapper::do_MemRegion(%p,%p)-gen(%d)",mr.start(),mr.end(), genid);
    ...
    while (entry >= first_entry) {
      ...
      if (!clear_card(entry)) { //this is where we will look at it.
...
void CardTableRS::clear_into_younger(Generation* gen, bool clear_perm) {
  ...
  if(TransGC)
  	{
	////by tony 2010-08-24
	  for(Generation* prev_gen = gch->prev_gen(g); g != NULL;)  //by this, we include gen0
	 {
  ...
  template <class T> void do_oop_work(T* p) {
    ...
    if (jp >= _begin && jp < _end) {
	  ...
	  if(!(obj == NULL ||
                ...
                (HeapWord*)obj >= _boundary)&&DebugTypeGenGC)
	  	{
	  		tty->print_cr("obj(%p)in gen(%i) was pointed by *p(%p) in gen(%i), _boundary=%p.",(HeapWord*)obj,Universe::heap()->addr_to_gen_id(obj),jp,Universe::heap()->addr_to_gen_id(jp),_boundary);

			gclog_or_tty->print_cr("obj(%p)in gen(%i) was pointed by *p(%p) in gen(%i), _boundary=%p.",(HeapWord*)obj,Universe::heap()->addr_to_gen_id(obj),jp,Universe::heap()->addr_to_gen_id(jp),_boundary);
...
public:
  ...
  //tony-> 2010-08-24 Collecting transEdens
	if(!TransGC)
	{
	    // Skip the youngest generation.
	    if (gen->level() == 0) return;  
	}
  //now we should also verify the young generation if we are using transGC.
...
void CardTableRS::verify_space(Space* s, HeapWord* gen_boundary) {
  ...
  if (s->end() <= gen_boundary){ 
	...
	if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY]s is under the gen_boundary, no verify for this space ending at:%p",s->end());}
	 //<-tony 2010-08-26
	return;
  	}
  ...
  //tony-> 2010-08-26 Fixing transEdens collector
  //when we verify the gen0, and eden()->used_region() is empty, the used.last()=used.first()+word_size-1=used.first()-1
  //and because used.first() is the begining of the whole heap, so used.last() falls out of the heap range.
  //that's why we received an assertion violation when we call as below:
  //jbyte* limit = byte_after(used.last());

  //to solve this problem, just need to check if the used space is empty, if so, return without any check.

  if(used.is_empty())
  	return;//by tony 2010-08-26
  ...
  if (ch->kind() == CollectedHeap::GenCollectedHeap) {
    ...
    if (cp->is_mark_sweep_policy() || cp->is_concurrent_mark_sweep_policy()) {
	 pg_boundary = gch->get_gen(1)->reserved().start();
		 ...
		 if(TransGC)
		 	{//we need to shift the boundary to the begining of the old for gen(2) is actually old gen in TransGC
		  pg_boundary = GenCollectedHeap::heap()->get_gen(2)->reserved().start();
		 	}
		  //<-tony 2010-08-19/
