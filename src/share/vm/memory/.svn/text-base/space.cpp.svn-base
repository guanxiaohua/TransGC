...
ContiguousSpace::ContiguousSpace(): CompactibleSpace(), _top(NULL),
    _concurrent_iteration_safe_limit(NULL),_saved_top(NULL) {
...
HeapWord* CompactibleSpace::forward(oop q, size_t size,
 ...
 //tony-> 2010-09-13 skip the transNewGen if it's the selected compaction destination
	if((cp->gen != NULL)&&(cp->gen->kind()==Generation::TransNew)){
		//if transNewGen is selected, we just continue to the previous gen, which should be the defNewGen //by tony 2010-09-13
		      cp->gen = GenCollectedHeap::heap()->prev_gen(cp->gen);		
		}
  //<-tony 2010-09-13
...
void Space::print_short_on(outputStream* st) const {
  st->print(" space " SIZE_FORMAT "K,"SIZE_FORMAT"K=%3d%% used", capacity() / K,used()/K,
...
void TransEden::reset(){
state=TransEden::Available;
transID=-1;
threadCount=0;
overflowed=false; 
 ...
 //<-tony 2011-1-16
////tony FIXME:Maybe it's erronous, but let's try.
//clear(SpaceDecorator::Mangle); //by tony 2010-09-16, disabling clear the space unconditionally

//during the MarkSweep, the transEden is reset;

//during a remote minor collection, we may need to clear the space.
...
void TransEden::print_on(outputStream* st) const {
  print_short_on(st);
  if(state==TransEden::Collectable)
  		st->print_cr(" [" INTPTR_FORMAT ", " INTPTR_FORMAT ", " INTPTR_FORMAT ") %s(%d) tc(%d)",
        		        bottom(), top(), end(), get_state_string(),transID,threadCount);
  else
  		st->print_cr(" [" INTPTR_FORMAT ", " INTPTR_FORMAT ", " INTPTR_FORMAT ") %s(%d)",
        		        bottom(), top(), end(), get_state_string(),transID);
  	
}
 //for print_on
 const char* TransEden::get_state_string()const{
 	switch(state)
			{
		case TransEden::Available:
			return "Available";
		case TransEden::Assigned:
				if(overflowed)
					{
					if(trained)
						return "Assigned,OF,TRAINED";
					else
						return "Assigned,OF";
					}
				else
					{
					if(trained)
						return "Assigned,TRAINED";
					else
						return "Assigned";
					}
		case TransEden::Full:
			return "Full";//we actually don't use this one
		case TransEden::Collectable:
				if(overflowed)
					{
					if(trained)
						return "Collectable,OF,TRAINED";
					else
						return "Collectable,OF";
					}
				else
					{
					if(trained)
						return "Collectable,TRAINED";
					else
						return "Collectable";
					}		
		case TransEden::Collecting:
			return "Collecting"; //used by gc
		default:
			return "Other";
...
HeapWord* ContiguousSpace::block_start_const(const void* p) const {
 ...
 //tony-> 2010-12-01 printing out the allocation site of the residuals.
  void ContiguousSpace::examine_alloc_site_of_objects()
 	{
		...
		tty->print_cr("examine_alloc_site_of_objects"); 	
