...
Generation::Generation(ReservedSpace rs, size_t initial_size, int level) :
  ...
  if(DebugTypeGenGC)
		  gclog_or_tty->print_cr("Generation %d",level);
  ...
  if(DebugTypeGenGC)
		  gclog_or_tty->print_cr("Generation %d done",level);
}
...
//by tony 2010-07-29
  TransNewGeneration*  Generation::as_TransNewGeneration(){
  assert((kind() == Generation::TransNew),
    "Wrong youngest generation type");
  return (TransNewGeneration*)this;
}
...
void Generation::oop_iterate(OopClosure* cl) {
 ...
 // used in transNewGeneration collection, when we are collecting transEden X, we don't need to iterate the oop's in X
 // this is achieved by the space_iteration in transNewGeneration, where we check if the space's status is collecting.
 //if so, skip it.
void Generation::oop_iterate_with_some_excluded(OopClosure* cl) {
  GenerationOopIterateClosure blk(cl, _reserved);
  if(kind()!=Generation::TransNew)
  		space_iterate(&blk);//go the normal way
  else
	  as_TransNewGeneration()->space_iterate_with_some_excluded(&blk);
}
