...
int Universe::_remote_obj_count = 0;
int Universe::_local_obj_count  = 0;
unsigned long long Universe::_remote_obj_bytes = 0;
unsigned long long Universe::_local_obj_bytes  = 0;
int Universe::_total_obj_count = 0;
unsigned long long Universe::_total_obj_bytes = 0;
double Universe::_space_ratio = 1.0; 
int Universe::_transParentThreadID=-1;
bool Universe::_trans_class_found=false;
int Universe::_newTransID=-1;
int Universe::_total_residual_count=0;
bool Universe::_training_done=false;
/*-tony 01/18/2010 */
...
jint Universe::initialize_heap() {
  ...
  if (UseParallelGC) {
    ...
    } else if (UseTypeGenGC) {  //Added by tony. 2010-1-17
      gc_policy = new TypeGenCollectorPolicy();
     if(DebugTypeGenGC)
	 	gclog_or_tty->print_cr("Using TypeGenGC!");
    ...
    Universe::_collectedHeap = new GenCollectedHeap(gc_policy);
     ...
     if(DebugTypeGenGC)
		  gclog_or_tty->print_cr("going to initialize heap");


  jint status = Universe::heap()->initialize();
     if(DebugTypeGenGC)
		  gclog_or_tty->print_cr("heap initialized!");
