...
class CardTableModRefBS: public ModRefBarrierSet {
 ...
 protected:
  ...
  if(TransGC&&!_whole_heap.contains(p)&&DebugTypeGenGC)
  	{
	gclog_or_tty->print_cr("[TONY] out of bounds access to card marking array, the address:%p, the heap(%p,%p)",p,_whole_heap.start(),_whole_heap.end());
	tty->print_cr("[TONY] out of bounds access to card marking array, the address:%p",p);
  	}
