...
//  assert(g->level() == 0, "Optimized for youngest gen.");
  assert( g->level() == 0 || (TransGC && g->level()==1 ), "Optimized for youngest gen.");  //modified by Feng. Feb, 2006
 ...
 /*-tony 2010-06-14 */
{
 ...
 //tony-> 2010-08-26 Fixing transEdens collector
  _lower_boundary=_g->reserved().start();
  _upper_boundary=_g->reserved().end();
 //<-tony 2010-08-26
...
//to avoid the generation name being repeated in the perfData, we introduce a static field.
 static bool generation0=true;
 /*-tony 01/18/2010 */
 	  
  // Generation counters -- generation 0, 3 subspaces
   if(TransGC)
  {_gen_counters = new GenerationCounters("new", generation0?0:1, 3, &_virtual_space);
   
  _gc_counters = new CollectorCounters(policy, generation0?0:1);
   	}
   else
   	{
 ...
 if(TransGC)
  generation0=false; //we support at most two defNewGenerations in the heap.
 /*-tony 01/18/2010 */
...
void DefNewGeneration::compute_space_boundaries(uintx minimum_eden_size,
  ...
  /*
  if (DebugTypeGenGC) { 
    tty->print_cr("new_arena %d Eden %p %p",1, eden_start, from_start);
...
/*
  ...
  if(UseTypeGenGC){
    size_t size_ratio = (size_t)Universe::space_ratio();

    if(level()==0)
      desired_new_size = old_size/NewRatio*size_ratio/(size_ratio+1);
    else if (level()==1)
      desired_new_size = old_size/NewRatio*1/(size_ratio+1);
  }
  else
   { desired_new_size = old_size/NewRatio; }
*/
 /*-tony 02/06/2010 */
...
HeapWord* rest=eden()->par_allocate(word_size);
