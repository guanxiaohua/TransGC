...
class Universe: AllStatic {
 ...
 private:
	...
	//statistics for the TypeGen
	
    //a flag for identifying the transaction point
  static int _transParentThreadID; //the tcp connection server thread.
  static int _newTransID; //the last assigned transaction ID.
  
  static int        _remote_obj_count;
  static int        _local_obj_count;
  static unsigned long long     _remote_obj_bytes;
  static unsigned long long    _local_obj_bytes;
  static int 	    _total_obj_count;
  static int 	    _total_residual_count;//by tony 2010-11-04 analysing the residuals
  static bool 		_training_done;
  static bool 		_trans_class_found;//by tony 2010-11-22 adding the _is_traced flag in Klass, indicating to the run time tracer whether or not to trace method call of this class.
  static unsigned long long _total_obj_bytes;
  static double _space_ratio;
	 /*-tony 01/18/2010 */
 ...
 public:
  ...
  //The following functions are moved to inline.hpp
  static inline int getNewTransID();
  static inline void setFoundTransClass();
  static inline bool isFoundTransClass();
  static inline bool isFoundTCPAcceptThread();
  static inline int getTCPAcceptThreadID();
  
  static inline void setTCPAcceptThreadID(int id);
  
  static inline void add_remote_obj_count(int val);//   { _remote_obj_count += val ;}
  static inline void add_local_obj_count(int val);//    { _local_obj_count += val ;}
  static inline  int  remote_obj_count();//              { return _remote_obj_count; }
  static inline int  local_obj_count();//               { return _local_obj_count; }

  static inline void add_remote_obj_bytes(int val);//   { _remote_obj_bytes += val ;}
  static inline void add_local_obj_bytes(int val);//    { _local_obj_bytes += val ;}
  static inline unsigned long long  remote_obj_bytes();//              { return _remote_obj_bytes; }
  static inline unsigned long long  local_obj_bytes();//               { return _local_obj_bytes; }

  static inline void add_total_obj_bytes(int val);
  static inline void add_total_obj_count(int val);
  static inline unsigned long long  total_obj_bytes();
  static inline int  total_obj_count();

  static void set_space_ratio(double ratio) {  _space_ratio = ratio; }
  static double space_ratio() { return _space_ratio;}
  

	static	inline void add_total_residual_count(int val) ;
	static	inline int total_residual_count() ;
	static inline bool isTrainingDone();
    static inline void setTrainingDone();
  /*-tony 01/18/2010 */
