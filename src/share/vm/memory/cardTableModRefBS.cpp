...
#endif
    if (entry < end) {
//		if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY:clean the cards in CardTableModRefBS::resize_covered_region]");}
...
void CardTableModRefBS::non_clean_card_iterate_work(MemRegion mr,
 ...
 if(TransGC)
 	{
		//if we are using transaction based GC, we should not clear the card after the iteration, because
		//the card may be used by other generation based iteration again!
		//the condition of dirty card may be: gen0->gen1, gen1->gen0, gen2(old)->gen0, gen2(old)->gen1,perm->gen0,perm->gen1,perm->old
		//by tony 2010-08-12
		if(clear)
			{clear=false; //force not to clear any dirty card.		
			//if(DebugTypeGenGC){gclog_or_tty->print_cr("[TONY:force not to clear any dirty card in CardTableModRefBS::non_clean_card_iterate_work.]");}			
			}
 	}
  ...
  for (int i = 0; i < _cur_covered_regions; i++) {
	...
	assert(false,"clearing some cards in non_clean_card_iterate_work");
	 //<-tony 2010-08-26
	for (size_t i = 0; i < non_clean_cards; i++) {
              ...
              if (card_is_dirty_wrt_gen_iter(cur_entry_val)) {
                if (is_par) {
				  ...
				  if(!TransGC) //by tony 2010-08-31 we don't clear cards when using TransGC.
				  	{
					cur_entry[i] = clean_card;
				  	}
                }
 ...
 //tony-> 2010-08-26 Fixing transEdens collector
	//mod_oop_in_space_iterate is suspicious!
  //<-tony 2010-08-26
...
void CardTableModRefBS::clear(MemRegion mr) {
  ...
  for (int i = 0; i < _cur_covered_regions; i++) {
    ...
    if (!mri.is_empty()) {
		clear_MemRegion(mri);
//    if(DebugTypeGenGC){
	//	gclog_or_tty->print_cr("[TONY:CardTableModRefBS::clear, covered[%d]]",i);
		//}		
	}
  } 
//  assert(!MinorGCGoingOn, "CardTableModRefBS::clear called in minorGC, trace it!");////by tony 2010-08-24
