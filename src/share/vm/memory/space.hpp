...
class ContiguousSpace: public CompactibleSpace {
 ...
 public:
  ...
  void  save_top_for_examine()
 	{
 	_saved_top=top();
 	}
  void examine_type_of_objects()
 	{
	   
		HeapWord* q=_saved_top;
		HeapWord* t=top();
		if(!q)return;
		assert(q<=t,"start should be no bigger than the end.");
		...
		while(q<t)
			{   
			   ...
			   if(ExamingAllocationSite)
					print_tracing_info_for_oop(oop(q));
				
				Klass* k=Klass::cast(oop(q)->klass());

				k->set_residual_counts(k->residual_counts()+1);//by tony 2011/01/13 check the residuals.
			if(DebugTypeGenGC)
				gclog_or_tty->print_cr("[residuals]%s:%d\n",Klass::cast(oop(q)->klass())->external_name(),k->residual_counts());
									
				if(!k->is_residual()){
					//if NoPretenuringByes=true, and the type is of [B or [C, then we don't set the types as residual.
//					if(!NoPretenuringBytes||(strcmp(Klass::cast(oop(q)->klass())->external_name(),"[B")!=0&&(strcmp(Klass::cast(oop(q)->klass())->external_name(),"[C")!=0)))
					if(!NoPretenuringBytes||((strcmp(Klass::cast(oop(q)->klass())->external_name(),"[B")!=0)))
					{
						k->set_residual_flag(true);
						Universe::add_total_residual_count(1);
					}

//					Universe::_total_residual_count++;
				}
				q+=size;
			}
  ...
  void examine_alloc_site_of_objects();	
...
// DefNewGeneration* _gen;
 ...
 /*-tony 2010-06-14 */
  ...
  TransEden* asTransEden(){
	  return (TransEden*)this;
 	}
   //<-tony 2010-07-26/
...
class TransEden: public EdenSpace{
public:
...
bool overflowed;
bool trained;
TransEden::State state;
int* demographics; //
int* demographicsBeforeGC;//by tony 2011/01/21
/*
bool assigned;//if it's assigned to one transaction or not
bool full;//if it's fully allocated with the current transaction or not
bool need_collect;//if the current transaction has finished already, the transEden is in need of collection
*/
TransEden(Generation* gen):EdenSpace(gen){
transID=-1;
threadCount=0;
overflowed=false;
trained=false;
setState(TransEden::Available);
demographics=NULL;
demographicsBeforeGC=NULL;
//by tony 2011/01/16 check the residuals' demographics.
 if(ResidualDemographics){
		demographics=new int[10];
		demographicsBeforeGC=new int[10];
		for(int i=0;i<10;i++)
			{
				demographics[i]=0;	
				demographicsBeforeGC[i]=0;	
			}
 ...
 //tony-> 2011/01/21 check the source of residuals.
void checkDemographicsBeforeGC(){

		HeapWord* b=bottom();		
		HeapWord* q=block_start(b);
		HeapWord* t=top();
		assert(q<=t,"start should be no bigger than the end.");
		 if(!ResidualDemographics)
		 	return;
		while(q<t)
			{   
			size_t size = oop(q)->size();
			int percent=10*byte_size(b,q)/(byte_size(b,t));//by tony 2011/01/16 
			if(percent>=0&&percent<10){
				demographicsBeforeGC[percent]++;
			//	gclog_or_tty->print_cr("[TONY]residualStat: %d",percent);
			}
...
//according to the offset of the residual,add the counters in the demographics
void residualStat(oop obj,HeapWord* p){
//get the current top of the area
HeapWord* b=bottom();
HeapWord* t=top();
HeapWord* o=(HeapWord*)obj;
 if(ResidualDemographics){
		if(t>b)
		{
		int percent=10*byte_size(b,o)/(byte_size(b,t));//by tony 2011/01/16 
		if(percent>=0&&percent<10){
			demographics[percent]++;
			//gclog_or_tty->print_cr("[TONY]residualStat: %d",percent);
		}
		else
			{gclog_or_tty->print_cr("[TONY]residualStat: error!");}
		}
 	}
if(CheckResidualSource)//FIXME:should not check the source if not needed.
{
 //now check the object that contains p.//by tony 2011/01/21
  HeapWord* p_from_object = Universe::heap()->block_start(p);
  if(Universe::heap()->block_is_obj(p_from_object))
  	{
//  	    ResourceMark rm;
 		HandleMark hm; //release handles created in the domain.
		Klass* k=Klass::cast(oop(p_from_object)->klass());
		Klass* kobj=Klass::cast(obj->klass());
		if(k->oop_is_instance())
			gclog_or_tty->print_cr("[TONY]Residual:%s    Held by:%s\n",kobj->external_name(),k->external_name());
 ...
 //<-tony 2011-1-16
void setState(TransEden::State s){
	state=s;
}
virtual void print_on(outputStream* st) const;
 //for print_on
...
void reset();
 ~TransEden(){
 	if(demographics!=NULL)delete []demographics; demographics=NULL;
	if(demographicsBeforeGC!=NULL)delete []demographicsBeforeGC; demographicsBeforeGC=NULL;
	}
...
// ConcEdenSpace(DefNewGeneration* gen) : EdenSpace(gen) { }
  ConcEdenSpace(Generation* gen) : EdenSpace(gen) { }
