...
class CollectedHeap : public CHeapObj {
 ...
 // inline static HeapWord* common_mem_allocate_init(size_t size, bool is_noref, TRAPS);
  ...
  // Allocate an uninitialized block of the given size, or returns NULL if
  // this is impossible.
  inline static HeapWord* common_mem_allocate_noinit(KlassHandle klass, size_t size, bool is_noref, TRAPS);

  // Like allocate_init, but the block returned by a successful allocation
  // is guaranteed initialized to zeros.
  inline static HeapWord* common_mem_allocate_init(KlassHandle klass, size_t size, bool is_noref, TRAPS);
...
virtual int get_trans_eden_id(void* add){return -1;}
...
//  virtual int addr_to_arena_id(void* addr) = 0;
  ...
  virtual int addr_to_gen_id(void* addr) {return -1;};
...
class GCCauseSetter : StackObj {
 ...
 //tony-> 2010-12-04 printing out the allocation site of the residuals.
