...
void print_tracing_info_for_oop(oop q){
				if(q==NULL)return;
...
//				printf("Print[");
			    for (JvmtiEnvBase* env = it.first(); env != NULL; env = it.next(env)) {
							 ...
							 //JvmtiExport::post_print_trace_for_obj(env->tag_map()->env(), (jobject)(oop(q)));
   			                 jobject jobj=JNIHandles::make_local(q);
							 if(jobj){
							 //printf("oop(%p)->jobject(%p)->oop(%p)",(HeapWord*)q,(HeapWord*)jobj,(HeapWord*)JNIHandles::resolve_external_guard(jobj));
							 JvmtiExport::post_set_scanning_residuals(env->tag_map()->env(),true);							 
   			                 JvmtiExport::post_print_trace_for_obj(env->tag_map()->env(), jobj);
								
							 }
   			                // JvmtiExport::post_print_trace_for_obj(env->tag_map()->env(), (jobject)to_jobject(oop(q)));
  // 			                printf("]\n");
			    }
