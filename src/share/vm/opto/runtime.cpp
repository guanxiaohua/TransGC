...
// @@PENG Marker Stubs
address OptoRuntime::_marker_enter_Java                           = NULL;
address OptoRuntime::_marker_exit_Java                            = NULL;
 //<-tony 2011-02-28
...
# endif
 ...
 //tony-> 2011/02/28 adding new bytecodes
  // @@PENG
  gen(env, _marker_enter_Java              , marker_handler_Type          , SharedRuntime::markerenter      ,    0 , true , false, false );
 tty->print_cr("_marker_enter_Java:"PTR64_FORMAT,_marker_enter_Java);

  gen(env, _marker_exit_Java               , marker_handler_Type          , SharedRuntime::markerexit       ,    0 , true , false, false );

  tty->print_cr("_marker_exit_Java:"PTR64_FORMAT,_marker_exit_Java);
 //<-tony 2011-02-28
}
...
// @@PENG
const TypeFunc* OptoRuntime::marker_handler_Type() {	
  // create input type (domain)
  const Type **fields = TypeTuple::fields(2);
  fields[TypeFunc::Parms+0] = TypeRawPtr::NOTNULL;;  // JavaThread
  fields[TypeFunc::Parms+1] = TypeInt::INT;        // mid
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+2, fields);
