...
void Parse::do_one_bytecode() {
 ...
 //tony-> 2011/02/28 adding new bytecodes
  // @@PENG
  case Bytecodes::_markerenter:		
	tty->print_cr("[AS-JIT] Compile marker #%d (+)", iter().get_index_int());
	  do_marker_enter(iter().get_index_int());
  break;
  case Bytecodes::_markerexit:
	  do_marker_exit(iter().get_index_int());
  break;
  //<-tony 2011-02-28
...
#define marker_stub(op) CAST_FROM_FN_PTR(address, OptoRuntime::marker_##op##_Java())

// @@PENG
void Parse::do_marker(int mid, bool is_enter){		
	Node* midnode = intcon(mid);
	address handler = is_enter ? marker_stub(enter) : marker_stub(exit);	

	Node* thread = _gvn.transform(new (C, 1) ThreadLocalNode());
	kill_dead_locals();
	const TypePtr* no_memory_effects = NULL;

	tty->print_cr("parse2.cpp do_marker");//by tony 2011/02/28

	Node* c = make_runtime_call(RC_NO_LEAF,
								OptoRuntime::marker_handler_Type(),
								handler, NULL, no_memory_effects,
								thread, midnode);
}
