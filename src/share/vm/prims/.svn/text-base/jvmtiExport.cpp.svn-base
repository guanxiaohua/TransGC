...
//by tony 2010-12-01 printing out the allocation site of the residuals.
void JvmtiExport::post_set_scanning_residuals(JvmtiEnv* env,jboolean set)
{
  jvmtiEventReserved callback;
  //we are using these two reserved call backs to set our flag isScanningResiduals in the client heapTracker
	if(set)
	 callback = env->callbacks()->reserved77;
	else
	 callback = env->callbacks()->reserved72;		
...
void JvmtiExport::post_print_trace_for_obj(JvmtiEnv* env,jobject object)
{
  jvmtiEventReserved callback;
  jlong  tag=0;
  env->GetTag(object, &tag);
  	//FIXME: shoudl multiplex object_free again!!
  ...
  post_object_free(env, tag);

}

  
void JvmtiExport::post_object_free(JvmtiEnv* env, jlong tag) {
//disable it temporarily //by tony 2010-12-05 printing out the allocation site of the residuals.
 ...
 //tony-> 2010-12-06 printing out the allocation site of the residuals.
 JvmtiThreadEventTransition jet(Thread::current());
...
if(TransGC)
{
	if(!VMInited)return; //only monitor when vm_thread is inited.
	
	Thread* tonythread=(Thread*)thread;

   {SafeResourceMark rm; 
//   if(strcmp( mh()->klass_name()->as_C_string(),"Mtrace"))
   tty->print_cr("TONYSTUDY:TRANSGC [%s:(ID:%d(TID:%d )] Method Entry %s->%s",
                     JvmtiTrace::safe_get_thread_name(thread),thread->osthread()->thread_id(),tonythread->get_pthread_id(),
                     (mh() == NULL) ? "NULL" : mh()->klass_name()->as_C_string(),
                     (mh() == NULL) ? "NULL" : mh()->name()->as_C_string()); 
	return;
   }
...
/*-tony 08/31/2009*/
   ...
   //cannot print here diretly until use SafeResourceMark!
...
if(TransGC)
{
   ...
   {SafeResourceMark rm; 
//   if(strcmp( mh()->klass_name()->as_C_string(),"Mtrace"))
   tty->print_cr("TONYSTUDY:TRANSGC [%s:(ID:%d(TID:%d )] Method Exit %s->%s",
                     JvmtiTrace::safe_get_thread_name(thread),thread->osthread()->thread_id(),tonythread->get_pthread_id(),
                     (mh() == NULL) ? "NULL" : mh()->klass_name()->as_C_string(),
                     (mh() == NULL) ? "NULL" : mh()->name()->as_C_string()); 
   }
...
//  	  printf("[ALLOC]<b.oop:%p>",(HeapWord*)obj);
  _allocated->push(obj);
}
