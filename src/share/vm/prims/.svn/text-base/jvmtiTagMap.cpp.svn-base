...
void JvmtiTagMap::get_young_generation() {
  ...
  switch (ch->kind()) {
    case (CollectedHeap::GenCollectedHeap): {
		 ...
		 if(TransGC)
		      _young_gen = ((GenCollectedHeap*)ch)->get_gen(1)->reserved();//we care about the transNewGen more than local young.
		 else
		 	 //<-tony 2010-12-01
...
//	  printf("[ALLOC]Added:oop:%p+entry:%p",(HeapWord*)o,(HeapWord*)entry);
	} else {
      // no-op
    }
  } else {
    ...
    if (tag == 0) {
      ...
      entry->set_tag(tag);
  	  //printf("[ALLOC]Up:oop:%p+entry:%p",(HeapWord*)o,(HeapWord*)entry);
    }
  }
}
...
//  printf("[ALLOC]<JvmtiTagMap::get_tag:oop:%p>\n",(HeapWord*)klassOop_if_java_lang_Class(o));
  // for Classes get the tag from the klassOop
  return tag_for(this, klassOop_if_java_lang_Class(o));
      ...
      while (entry != NULL) {
        ...
        if (oop == NULL) {
          ...
          if (post_object_free) {
			...
			JvmtiExport::post_set_scanning_residuals(env(),false);
        ...
        if (same_gen) {
          ...
          if (post_object_free) {
		  	//by tony 2010-12-01 printing out the allocation site of the residuals.
			JvmtiExport::post_set_scanning_residuals(env(),true);
            JvmtiExport::post_object_free(env(), entry->tag());
          }
          */
        }
